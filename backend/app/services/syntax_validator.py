...
class SyntaxValidator:
    ...
    def _validate_csharp(code: str) -> ValidationResult:
        """
        C# pass-through — dotnet/csc compilation requires a full project context
        (.csproj, SDK resolution) that is impractical for inline snippet checking.

        validation_tool_available=False signals to the DB that this slot was not
        checked so reviewers know to scrutinise C# output manually.
        """
        logger.debug(
            "C# validation not implemented (requires full dotnet SDK project) — passing through"
        )
        return ValidationResult(valid=True, errors=[], tool_available=False)
